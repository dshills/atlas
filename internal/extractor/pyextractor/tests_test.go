...
func TestExtractTestReferences_ClassPrefixMatch(t *testing.T) {
	symbols := []extractor.SymbolRecord{
		{Name: "User", QualifiedName: "app.models.User", SymbolKind: "class"},
...
func TestExtractTestReferences_CaseInsensitive(t *testing.T) {
	symbols := []extractor.SymbolRecord{
		{Name: "ProcessOrder", QualifiedName: "app.orders.ProcessOrder", SymbolKind: "function"},
		{Name: "test_processorder", QualifiedName: "tests.test_orders.test_processorder", SymbolKind: "test", StartLine: 3},
	}

	refs := extractTestReferences(symbols, "app.orders")
