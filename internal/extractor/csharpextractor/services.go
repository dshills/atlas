...
var (
	csharpHttpClientRe = regexp.MustCompile(`(?m)\b(HttpClient|WebRequest|RestClient)\b`)
	csharpBgJobRe      = regexp.MustCompile(`(?m)\b(Task\.Run|new\s+Thread|BackgroundService|Parallel\.ForEach)\b`)
)
...
func extractServices(content string, _ []string, codeLines []bool) ([]extractor.ReferenceRecord, []extractor.ArtifactRecord) {
	...
	// HTTP clients
	for _, m := range csharpHttpClientRe.FindAllStringSubmatchIndex(content, -1) {
	...
	// Background jobs
	for _, m := range csharpBgJobRe.FindAllStringSubmatchIndex(content, -1) {
		name := content[m[2]:m[3]]
		line := strings.Count(content[:m[0]], "\n") + 1
		if line-1 >= len(codeLines) || !codeLines[line-1] {
			continue
