...
var (
	...
	javaStringLitRe = regexp.MustCompile(`(?s)"([^"]{20,})"`)
	jpaQueryRe      = regexp.MustCompile(`(?m)@Query\s*\(\s*(?:value\s*=\s*)?["']([^"']+)["']`)
)
...
func extractSQLArtifacts(content string, _ []string, filePath string, codeLines []bool) ([]extractor.ReferenceRecord, []extractor.ArtifactRecord) {
	...
	if strings.Contains(filePath, "migrations/") || strings.Contains(filePath, "migrate/") ||
		strings.Contains(filePath, "flyway/") || strings.Contains(filePath, "liquibase/") {
	...
	// Dedup by line number to avoid double-counting when both string literal
	// and JPA @Query regexes match the same SQL on the same line.
	seen := map[int]bool{}
	...
	for _, m := range javaStringLitRe.FindAllStringSubmatchIndex(content, -1) {
	...
	for _, m := range jpaQueryRe.FindAllStringSubmatchIndex(content, -1) {
		line := strings.Count(content[:m[0]], "\n") + 1
		if line-1 >= len(codeLines) || !codeLines[line-1] {
			continue
		}
